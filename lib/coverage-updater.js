...
function getCoverageSummaryPath(coverageDir, isKarma = false) {
  let browserDir;
  if (isKarma) {
    ...
    browserDir = path.join(coverageDir, coverageSummaryDir);
  } else {
    browserDir = '';
  }
  return path.join(coverageDir, browserDir, 'coverage-summary.json');
}
...
function updateCodeCoverage(options = {}) {
  ...
  const coverageSummaryPath = options.coverageSummaryPath ||
    getCoverageSummaryPath(coverageDir, options.isKarma);
