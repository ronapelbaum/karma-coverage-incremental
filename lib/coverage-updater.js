...
const ROOT = process.cwd() || path.join(ROOT, '..');
...
function updateCodeCoverage(options = {}) {
  const coveragePath = options.coveragePath || path.join(ROOT, 'coverage');
  const coverageSummaryPath = options.coverageSummaryPath || getCoverageSummaryPath(coveragePath);
